...
public class NewExceptions4 {
	...
	private static final Logger log = LoggerFactory.getLogger(NewExceptions4.class);
	...
	public synchronized void createNewRandomException() {
		...
		if (storedDate == 0 || storedDate < now) {
			storedDate = now;
			log.info("Its a brand new day - lets a brand new exceptions");
			int i = RandomUtil.getRandomNumberInRange(1, 5);
			if (i == 3) {
